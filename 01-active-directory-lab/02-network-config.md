...
Jeśli posiadamy zainstalowane już dwie maszyny, kolejnym krokiem powinna być zmiana ustawień sieciowych w VirtualBox dla poszczególnej maszyny.

Wchodząc w ustawienia sieciowe w VirtualBox, natkniemy się na kilka rodzajów połączeń:
...
Wchodzimy w naszą maszynę wirtualną na której zainstalowaliśmy Windows Server, aby ustawić odpowiednie IP dla naszego głównego serwera.
...
4. Zweryfikować czy adres jest wolny: `ping 192.168.1.10` — brak odpowiedzi = adres wolny

<img width="891" height="427" alt="image" src="https://github.com/user-attachments/assets/378544cd-65b9-4f2d-8179-420e715bf612" />

Osobiście wybrałem liczbę wysoką np. 200, aby uniknąć zbędnych problemów z konfiguracją.
Następnie zapisałem sobie wyżej wymienione rzeczy czyli: adres IP, maskę podsieci oraz bramę domyślną.

Zweryfikowany został adres IP poleceniem: `ping 192.168.x.x`

Następnie przechodzimy do panelu sterowania na naszej maszynie z Windows Server:

-> Network and Internet
-> Network and Sharing Center
-> Klikamy w niebieski link z nazwą naszej sieci
-> Klikamy właściwości
-> Następnie klikamy na Internet Protocol Version 4 (TCP/IPv4) -> Właściwości
-> Klikamy Use the Following IP Address (w tym momencie przypisujemy naszemu serwerowi statyczny adres IP)
-> Klikamy na pole Use the following DNS Server address

| Parametr | Wartość |
|----------|---------|
| IP address | Statyczny IP poza zakresem DHCP (np. `192.168.x.10`) |
| Subnet mask | Maska podsieci z `ipconfig` |
| Default gateway | Adres routera z `ipconfig` |
| Preferred DNS server | Ten sam co IP serwera |
...
-> Klikamy OK i w tym momencie mamy przydzielony statyczny adres IP do naszego serwera AD.

---

**3. Zmiana nazwy serwera**

Kolejno po przydzieleniu IP wchodzimy w Server Manager i klikamy w WorkGroup. Zakładka Computer Name i klikamy opcję "Zmień" lub "Change".

<img width="565" height="594" alt="image" src="https://github.com/user-attachments/assets/c549bed4-9d79-427d-8543-e9fcea0929d0" />

-> Zmieniamy nazwę, klikamy OK i restartujemy komputer.

**⚠️ WAŻNA UWAGA** — W tym kroku ustawiamy **tylko nazwę komputera** (np. `main-server-AD`).
Nazwa domeny `projekt.local` zostanie nadana dopiero podczas instalacji roli AD DS w kolejnym kroku.
